...
/**
 * @brief Confirms Application 2 as the successfully booted image.
 *
 * Slot B is confirmed when it is already the active slot or when it is the
 * pending slot selected by the bootloader. Confirmation updates metadata to
 * mark Slot B as active, clears pending/rollback state, and resets boot attempts.
 */
...
void App_Start()
{
    const ImageHeader_t *image_header = (const ImageHeader_t *)SLOT_B_BASE_ADDR;
